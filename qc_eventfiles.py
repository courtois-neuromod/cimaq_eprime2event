...
def get_all_tsvs(in_dir):
    ...
    tsv_list = sorted(glob.glob(os.path.join(in_dir,
                                             'sub*task-memory_events.tsv')))
...
def qc_tsv_file(tsv_path):
    ...
    try:
        ...
        missing_cols = []
        col_list = ['trial_number', 'onset', 'duration', 'offset',
                    'trial_type', 'response',  'response_time', 'stim_id',
                    'stim_file', 'stim_category', 'stim_name',
                    'recognition_accuracy', 'recognition_responsetime',
                    'position_correct', 'position_response',
        ...
        trial_num = np.array_equal(dframe['trial_number'].tolist(),
                                   range(4, 121))
        if not trial_num:
        ...
        # Check assigment of stimulus names and categories
        # from post-scan output file
...
def main():
    ...
    tsv_list = get_all_tsvs(in_dir)

    # text file that documents trials with no response, per run
    qc_report = open(os.path.join(out_dir, 'eventfiles_QCreport.txt'), 'w+')
